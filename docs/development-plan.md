...
### 🛡️ Sprint 3: 예외 처리 6종 완비 & 인터랙션 구현 (Exception & Resilience)

* **목표**: `PRD.md` 5절에 명시된 6가지 예외 상황을 100% 충실히 처리
* **주요 과제**:
  1. **[5-2] 보안과 무관하거나 존재하지 않는 용어**:
     - AI 판별을 통해 무관한 용어 감지 시 "'{입력값}'은(는) 보안 용어로 보이지 않아요." 안내
     - 추천 예시 카드 (예: "피싱", "VPN", "제로데이") 노출 및 클릭 시 즉시 재검색
  2. **[5-3] 오탈자 보정 및 추천**:
     - 유사 보안 용어 추천 ("혹시 '{추정용어}'를 찾으시나요?" 버튼 제시 또는 자동 보정)
  3. **[5-4] AI/API 생성 실패 대응**:
     - 부분/손상된 결과 감지 및 에러 상태 전환
     - "설명을 불러오지 못했어요. 다시 시도해주세요." + "다시 시도" 버튼 제공
  4. **[5-6] 결과 재시도 요구 (다른 설명으로 다시 보기)**:
     - 결과 카드 하단에 "다른 설명으로 다시 보기" 버튼 상시 제공
     - 동일 용어에 대해 AI 재생성 요청 및 기존 결과 스무스한 교체
